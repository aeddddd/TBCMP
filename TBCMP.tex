...
在这一节我们关注的基本点是: 一个态右矢如何随时间改变? 假定我们有一个物理系统,它的态右矢在 ${t}_{0}$ 时刻由 $|\alpha \rangle$ 表示. 在稍后的时刻,一般而言,我们不要指望该系统保持在同样的状态 $|\alpha \rangle$ . 让我们把某稍后时刻与该状态相对应的右矢表示为

$$
\left| {\alpha ,{t}_{0};t}\right\rangle ,\;\left( {t > {t}_{0}}\right) , tag{2.1.1}
$$

其中标出 $\alpha ,{t}_{0}$ 是为了提醒我们自己,该系统在某个较早的参考时刻 ${t}_{0}$ 曾经处于态 $|\alpha \rangle$ . 由于时间被认为是一个连续参量, 我们预期

$$
\mathop{\lim }\limits_{{t \rightarrow {t}_{0}}}\left| {\alpha ,{t}_{0};t}\right\rangle = |\alpha \rangle , tag{2.1.2}
$$

但是, 我们也可以用一种简化符号,

$$
\left| {\alpha ,{t}_{0};{t}_{0}}\right\rangle = \left| {\alpha ,{t}_{0}}\right\rangle , tag{2.1.3}
$$

代替它. 我们的基本任务是研究一个态右矢的时间演化:

$$
\left| {\alpha ,{t}_{0}}\right\rangle = |\alpha \rangle \xrightarrow[]{\text{ 时间演化 }}\left| {\alpha ,{t}_{0};t}\right\rangle . tag{2.1.4}
$$

换成另一种表达方式,我们感兴趣的是,探寻在时间位移变换 ${t}_{0} \rightarrow t$ 下,态右矢如何变化.

就像平移的情况一样,这两个右矢由一个被称之为时间演化算符的 $u\left( {t,{t}_{0}}\right)$ 联系起来:

$$
\left| {\alpha ,{t}_{0};t}\right\rangle = u\left( {t,{t}_{0}}\right) \left| {\alpha ,{t}_{0}}\right\rangle . tag{2.1.5}
$$

我们希望描述时间演化算符的哪些性质呢? 第一个重要的性质源于概率守恒对 $\mathbf{u}\left( {t,{t}_{0}}\right)$ 的幺正性的要求. 假定在 ${t}_{0}$ 时刻态右矢用某个可观测量 $A$ 的本征右矢展开:

$$
\left| {\alpha ,{t}_{0}}\right\rangle = \mathop{\sum }\limits_{{a}^{\prime }}{c}_{{a}^{\prime }}\left( {t}_{0}\right) \left| {a}^{\prime }\right\rangle . tag{2.1.6}
$$

同样地, 在稍后某个时刻, 我们有

$$
\left| {\alpha ,{t}_{0};t}\right\rangle = \mathop{\sum }\limits_{{a}^{\prime }}{c}_{{a}^{\prime }}\left( t\right) \left| {a}^{\prime }\right\rangle , tag{2.1.7}
$$

一般来说, 我们无法指望各个展开系数的模保持相同 * :

$$
\left| {{c}_{{a}^{\prime }}\left( t\right) }\right| \neq \left| {{c}_{{a}^{\prime }}\left( {t}_{0}\right) }\right| . tag{2.1.8}
$$

例如,考虑一个自旋 $\frac{1}{2}$ 系统,其自旋磁矩受到一个沿 $z$ 方向的均匀磁场的作用. 为确定起见,假定 ${t}_{0}$ 时刻该自旋沿正 $x$ 方向,即发现该系统处在 ${S}_{x}$ 的一个本征态上,其本征值为 $\hbar /2$ . 正如稍后将在本节中定量证明的,随着时间的推移,该自旋在 ${xy}$ 平面上进动. 这意味着,在 $t > {t}_{0}$ 时刻观测到 ${S}_{x}$ +的概率不再是 1,还有有限的概率观测到 ${S}_{x}$ -. 而在所有的时刻,观测到 ${S}_{x} +$ 和 ${S}_{x}$ 一的概率之和都保持为 1 . 尽管对于各个展开系数有 (2.1.8) 式, 采用 (2.1.6) 式和 (2.1.7) 式的符号, 一般我们一定有

$$
\mathop{\sum }\limits_{{a}^{\prime }}{\left| {c}_{{a}^{\prime }}\left( {t}_{0}\right) \right| }^{2} = \mathop{\sum }\limits_{{a}^{\prime }}{\left| {c}_{{a}^{\prime }}\left( t\right) \right| }^{2} tag{2.1.9}
$$

另一种表述方式是, 如果态右矢初始时归一到了 1 , 则在所有以后的时刻它一定保持归一到 1 :

$$
\left\langle {\alpha ,{t}_{0} | \alpha ,{t}_{0}}\right\rangle = 1 \Rightarrow \left\langle {\alpha ,{t}_{0};t | \alpha ,{t}_{0};t}\right\rangle = 1. tag{2.1.10}
$$

就像在平移的情况一样, 如果时间演化算符取为幺正算符, 这一性质就能确保. 为此, 我们将幺正性,

$$
{u}^{ + }\left( {t,{t}_{0}}\right) u\left( {t,{t}_{0}}\right) = 1, tag{2.1.11}
$$

取为 $u$ 算符基本性质中的一个. 它与许多作者把幺正性视为概率守恒的同义词并非巧合.

我们要求 $u$ 算符具有的另外一个特征是它的结合性:

$$
u\left( {{t}_{2},{t}_{0}}\right) = u\left( {{t}_{2},{t}_{1}}\right) u\left( {{t}_{1},{t}_{0}}\right) ,\;\left( {{t}_{2} > {t}_{1} > {t}_{0}}\right) . tag{2.1.12}
$$

这个方程是说,如果对获得从 ${t}_{0}$ 到 ${t}_{2}$ 的时间演化感兴趣,我们则可以通过考虑先从 ${t}_{0}$ 到 ${t}_{1}$ ,再从 ${t}_{1}$ 到 ${t}_{2}$ 的时间演化得到相同的结果一一这是一个合理的要求. 注意我们要从右向左读 (2.1.12) 式!

考虑一个无穷小时间演化算符 $u\left( {{t}_{0} + {dt},{t}_{0}}\right)$ 总是十分有益的:

$$
\left| {\alpha ,{t}_{0};{t}_{0} + {dt}}\right\rangle = u\left( {{t}_{0} + {dt},{t}_{0}}\right) \left| {\alpha ,{t}_{0}}\right\rangle . tag{2.1.13}
$$

由于连续性 [见 (2.1.2) 式],当 ${dt}$ 趋于零时,该无穷小时间演化算符一定约化为单位算符,

$$
\mathop{\lim }\limits_{{{dt} \rightarrow 0}}u\left( {{t}_{0} + {dt},{t}_{0}}\right) = 1, tag{2.1.14}
$$

而且,就像在平移情况中那样,我们预期 $u\left( {{t}_{0} + {dt},{t}_{0}}\right)$ 与 1 之差为 ${dt}$ 的量级.

我们断言, 下式满足所有这些要求

$$
u\left( {{t}_{0} + {dt},{t}_{0}}\right) = 1 - {i\Omega dt}, tag{2.1.15}
$$

其中 $\Omega$ 是一个厄米算符**

$$
{\Omega }^{ \dagger } = \Omega \text{.} tag{2.1.16}
$$

使用 (2.1.15) 式时, 该无穷小时间平移算符满足结合性

$$
u\left( {{t}_{0} + d{t}_{1} + d{t}_{2},{t}_{0}}\right) = u\left( {{t}_{0} + d{t}_{1} + d{t}_{2},{t}_{0} + d{t}_{1}}\right) u\left( {{t}_{0} + d{t}_{1},{t}_{0}}\right) ; tag{2.1.17}
$$

它与单位算符只相差一个 ${dt}$ 量级的项. 幺正性还可以在 ${\left( dt\right) }^{2}$ 或更高阶的项可以忽略的

* 然而,稍后我们将证明,如果哈密顿量与 $A$ 对易,则 $\left| {{c}_{{a}^{\prime }}\left( t\right) }\right|$ 的确等于 $\left| {{c}_{{a}^{\prime }}\left( {t}_{0}\right) }\right|$ .

** 如果这个 $\Omega$ 算符显含时间,那时它必须在 ${t}_{0}$ 时刻求值. 程度上被验证

$$
{\mathbf{u}}^{ \dagger }\left( {{t}_{0} + {dt},{t}_{0}}\right) \mathbf{u}\left( {{t}_{0} + {dt},{t}_{0}}\right) = \left( {1 + i{\Omega }^{ \dagger }{dt}}\right) \left( {1 - {i\Omega dt}}\right) \simeq 1. tag{2.1.18}
$$

算符 $\Omega$ 具有频率或者反比于时间的量纲. 那么存在我们熟悉的具有频率量纲的可观测量吗? 回忆一下,在旧量子论中,假定角频率 $\omega$ 通过普朗克-爱因斯坦关系

$$
E = \hbar \omega . tag{2.1.19}
$$

与能量联系起来. 现在让我们借用经典力学中哈密顿量是时间演化生成元的概念 (Goldstein ${2002},{401} \sim {402}$ 页). 那时自然可以把 $\Omega$ 与哈密顿量算符 $H$ 联系起来:

$$
\Omega = \frac{H}{\hbar }. tag{2.1.20}
$$

综上所述, 无穷小时间演化算符可写为:

$$
u\left( {{t}_{0} + {dt},{t}_{0}}\right) = 1 - \frac{iHdt}{\hbar }, tag{2.1.21}
$$

其中假定了哈密顿量算符 $H$ 是厄米的. 读者可能要问,这里引入的 $\hbar$ 是否与出现在平移算符表达式 (1.6.32) 中的 $\hbar$ 相同. 人们可通过稍后导出的量子力学运动方程与经典运动方程做比较回答这个问题. 结果表明,除非这两个 $\hbar$ 是一样的,否则就不能够得到作为相应量子力学关系式的经典极限关系式

$$
\frac{d\mathbf{x}}{dt} = \frac{\mathbf{p}}{m} tag{2.1.22}
$$

\subsection*{薛定谔方程}

我们现在能够推导时间演化算符 $\mathbf{u}\left( {t,{t}_{0}}\right)$ 满足的基本微分方程. 在 (2.1.12) 式中令 ${t}_{1} \rightarrow t,{t}_{2} \rightarrow t + {dt}$ ,利用时间演化算符的结合性:

$$
u\left( {t + {dt},{t}_{0}}\right) = u\left( {t + {dt}, t}\right) u\left( {t,{t}_{0}}\right) = \left( {1 - \frac{iHdt}{\hbar }}\right) u\left( {t,{t}_{0}}\right) , tag{2.1.23}
$$

其中的时间差 $t - {t}_{0}$ 不必为无穷小. 我们有

$$
u\left( {t + {dt},{t}_{0}}\right) - u\left( {t,{t}_{0}}\right) = - i\left( \frac{H}{\hbar }\right) {dt}u\left( {t,{t}_{0}}\right) , tag{2.1.24}
$$
...
$$
\frac{d{A}^{\left( H\right) }}{dt} = \frac{\partial {u}^{ \dagger }}{\partial t}{A}^{\left( S\right) }u + {u}^{ \dagger }{A}^{\left( S\right) }\frac{\partial u}{\partial t}
$$
...
$$
\frac{d{A}^{\left( H\right) }}{dt} = \frac{1}{i\hbar }\left\lbrack {{A}^{\left( H\right) }, H}\right\rbrack . tag{2.2.19}
$$
...
$$
\frac{dA}{dt} = {\left\lbrack A, H\right\rbrack }_{\text{经典 }}. tag{2.2.20}
$$
...
$$
\frac{d{S}_{i}^{\left( H\right) }}{dt} = \frac{1}{i\hbar }\left\lbrack {{S}_{i}^{\left( H\right) }, H}\right\rbrack , tag{2.2.21}
$$
...
$$
\frac{d{p}_{i}}{dt} = \frac{1}{i\hbar }\left\lbrack {{p}_{i}, H}\right\rbrack = 0. tag{2.2.25}
$$
...
$$
\frac{d{x}_{i}}{dt} = \frac{1}{i\hbar }\left\lbrack {{x}_{i}, H}\right\rbrack = \frac{1}{i\hbar }\frac{1}{2m}i\hbar \frac{\partial }{\partial {p}_{i}}\left( {\mathop{\sum }\limits_{{j = 1}}^{3}{p}_{j}^{2}}\right) tag{2.2.26}
$$
...
$$
\frac{d{p}_{i}}{dt} = \frac{1}{i\hbar }\left\lbrack {{p}_{i}, V\left( \mathbf{x}\right) }\right\rbrack = - \frac{\partial }{\partial {x}_{i}}V\left( \mathbf{x}\right) , tag{2. 2.32}
$$

另一方面,我们看到因为 ${x}_{i}$ 与新添加的 $V\left( \mathbf{x}\right)$ 项对易,

$$
\frac{d{x}_{i}}{dt} = \frac{{p}_{i}}{m} tag{2. 2.33}
$$

仍然成立. 我们可以再一次利用海森堡运动方程推导出:

$$
\frac{{d}^{2}{x}_{i}}{d{t}^{2}} = \frac{1}{i\hbar }\left\lbrack {\frac{d{x}_{i}}{dt}, H}\right\rbrack = \frac{1}{i\hbar }\left\lbrack {\frac{{p}_{i}}{m}, H}\right\rbrack tag{2. 2.34}
$$

$$
= \frac{1}{m}\frac{d{p}_{i}}{dt},
$$
...
$$
m\frac{{d}^{2}}{d{t}^{2}}\langle \mathbf{x}\rangle = \frac{d\langle \mathbf{p}\rangle }{dt} = - \langle \nabla V\left( \mathbf{x}\right) \rangle . tag{2.3.36}
$$
...
$$
\frac{da}{dt} = \sqrt{\frac{m\omega }{2\hbar }}\left( {\frac{p}{m} - {i\omega x}}\right) = - {i\omega a}
$$
...
$$
\frac{d{a}^{ \dagger }}{dt} = {i\omega }{a}^{ \dagger }
$$
...
$$
\widetilde{G}\left( E\right) \equiv - i{\int }_{0}^{\infty }{dtG}\left( t\right) \exp \left( {{iEt}/\hbar }\right) /\hbar tag{2.6.23}
$$

$$
= - i{\int }_{0}^{\infty }{dt}\mathop{\sum }\limits_{{a}^{\prime }}\exp \left( {-i{E}_{{a}^{\prime }}t/\hbar }\right) \exp \left( {{iEt}/\hbar }\right) /\hbar ,
$$
...
$$
\delta {\int }_{{t}_{1}}^{{t}_{2}}{dt}{L}_{\text{经典 }}\left( {x,\dot{x}}\right) = 0, tag{2.6.35}
$$
...
$$
\exp \left\lbrack {i{\int }_{{t}_{1}}^{{t}_{2}}\frac{{dt}{L}_{\text{经典 }}\left( {x,\dot{x}}\right) }{\hbar }}\right\rbrack \text{ 对应于 }\left\langle {{x}_{2},{t}_{2} | {x}_{1},{t}_{1}}\right\rangle .
$$
...
$$
S\left( {n, n - 1}\right) \equiv {\int }_{{t}_{n - 1}}^{{t}_{n}}{dt}{L}_{\text{经典 }}\left( {x,\dot{x}}\right) . tag{2.6.36}
$$
...
$$
S\left( {n, n - 1}\right) = {\int }_{{t}_{n - 1}}^{{t}_{n}}{dt}\left\lbrack {\frac{m{\dot{x}}^{2}}{2} - V\left( x\right) }\right\rbrack tag{2.6.41}
$$
...
$$
\left\langle {{x}_{N},{t}_{N} | {x}_{1},{t}_{1}}\right\rangle = {\int }_{{x}_{1}}^{{x}_{N}}\mathcal{D}\left\lbrack {x\left( t\right) }\right\rbrack \exp \left\lbrack {i{\int }_{{t}_{1}}^{{t}_{N}}{dt}\frac{{L}_{\text{经典 }}\left( {x,\dot{x}}\right) }{\hbar }}\right\rbrack . tag{2.6.49}
$$
...
$$
{\phi }_{1} - {\phi }_{2} = \left( \frac{1}{\hbar }\right) {\int }_{{t}_{i}}^{{t}_{f}}{dt}\left\lbrack {{V}_{2}\left( t\right) - {V}_{1}\left( t\right) }\right\rbrack . tag{2. 7.9}
$$
...
$$
\left\langle {{\mathbf{x}}_{n},{t}_{n} | {\mathbf{x}}_{n - 1},{t}_{n - 1}}\right\rangle = \sqrt{\frac{m}{{2\pi i}\hbar {\Delta t}}}\exp \left\lbrack {i{\int }_{{t}_{n} - 1}^{{t}_{n}}{dt}\frac{\left( \frac{1}{2}m{\dot{\mathbf{x}}}^{2} - mgz\right) }{\hbar }}\right\rbrack ,
$$
...
$$
\delta {\int }_{{t}_{1}}^{{t}_{2}}{dt}\left( {\frac{m{\dot{\mathbf{x}}}^{2}}{2} - {mgz}}\right) = 0, tag{2.7.13}
$$
...
$$
\frac{d{x}_{i}}{dt} = \frac{\left\lbrack {x}_{i}, H\right\rbrack }{i\hbar } = \frac{\left( {p}_{i} - e{A}_{i}/c\right) }{m}, tag{2.7.22}
$$

它显示本书中定义为平移生成元的算符 $\mathbf{p}$ 与 ${md}\mathbf{x}/{dt}$ 不同. $\mathbf{p}$ 经常称为正则动量,以示与运动学 (或力学) 动量的区别,后者用 $\Pi$ 标记:

$$
\Pi \equiv m\frac{d\mathbf{x}}{dt} = \mathbf{p} - \frac{e\mathbf{A}}{c}. tag{2.7.23}
$$
...
$$
m\frac{{d}^{2}\mathbf{x}}{d{t}^{2}} = \frac{d\Pi }{dt} = e\left\lbrack {\mathbf{E} + \frac{1}{2c}\left( {\frac{d\mathbf{x}}{dt} \times \mathbf{B} - \mathbf{B} \times \frac{d\mathbf{x}}{dt}}\right) }\right\rbrack tag{2.7.27}
$$
...
$$
\frac{d{p}_{x}}{dt} = - \frac{\partial H}{\partial x},\;\frac{d{p}_{y}}{dt} = - \frac{\partial H}{\partial y},\cdots tag{2. 7.43}
$$

一般说来,正则动量 $\mathbf{p}$ 不是一个规范不变量,即使我们针对的是同一个物理情况,它的数值仍依赖于所采用的特殊规范. 相比之下,跟踪粒子轨迹的运动学动量 $\Pi$ ,或 ${md}\mathbf{x}/{dt}$ ,是一个规范不变量,这一点可以明确证明. 因为 $\mathbf{p}$ 和 ${md}\mathbf{x}/{dt}$ 通过 (2.7.23) 式相关联, $\mathbf{p}$ 必须改变以补偿 (2.7.42) 式给出的 $\mathbf{A}$ 的变化.
...
$$
{L}_{\text{经典 }}^{\left( 0\right) } = \frac{m}{2}{\left( \frac{d\mathbf{x}}{dt}\right) }^{2} \rightarrow {L}_{\text{经典 }}^{\left( 0\right) } + \frac{e}{c}\frac{d\mathbf{x}}{dt} \cdot \mathbf{A}. tag{2. 7.65}
$$

然后,对于从 $\left( {{\mathbf{x}}_{n - 1},{t}_{n - 1}}\right)$ 到 $\left( {{\mathbf{x}}_{n},{t}_{n}}\right)$ 的某段确定的路径单元,作用量相应的改变由下式给出

$$
{S}^{\left( 0\right) }\left( {n, n - 1}\right) \rightarrow {S}^{\left( 0\right) }\left( {n, n - 1}\right) + \frac{e}{c}{\int }_{{t}_{n - 1}}^{{t}_{n}}{dt}\left( \frac{d\mathbf{x}}{dt}\right) \cdot \mathbf{A}. tag{2. 7.66}
$$

而最后的这个积分可以写成

$$
\frac{e}{c}{\int }_{{t}_{n - 1}}^{{t}_{n}}{dt}\left( \frac{d\mathbf{x}}{dt}\right) \cdot \mathbf{A} = \frac{e}{c}{\int }_{{x}_{n - 1}}^{{x}_{n}}\mathbf{A} \cdot d\mathbf{s}, tag{2.7.67}
$$
...
$$
\frac{d}{dt}\langle \mathbf{x} \cdot \mathbf{p}\rangle = \left\langle \frac{{\mathbf{p}}^{2}}{m}\right\rangle - \langle \mathbf{x} \cdot \nabla V\rangle .
$$
