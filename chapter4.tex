...
\end{equation}
对于非静态的轨迹,其意味着作用量的大幅振荡,其平均值为0,或者说,更准确的,传播子$U(q_f,q_i;t_f-t_i)$由轨迹$q(t)$所主导,其作用量$S[q]$与经典作用量$S_c=S[q_c]$相差一个$\hbar$阶项:$|S-S_c|\lesssim\hbar$.当$|S_c|\gg\hbar $时,这些轨迹非常接近经典作用量的轨迹.而对于相反的极限中,同样满足条件$|S-S_c|\lesssim\hbar$的轨迹就与经典作用量导致的轨迹截然不同.于是,形式上的经典极限对于与$\hbar\to0$的极限.
